...
public class PremodifierTest {
	...
	public void indefiniteWithPremodifierTest() {
		...
		NPPhraseSpec np = this.phraseFactory.createNounPhrase("一", "个", "狭窄");
		s.setObject(np);
	 ...
	 */
	@Test
	public void testPremodifierOrder() {
		AdjPhraseSpec adv1 = this.phraseFactory.createAdjectivePhrase("绿色");
		AdjPhraseSpec adv2 = this.phraseFactory.createAdjectivePhrase("长");

		NPPhraseSpec np = this.phraseFactory.createNounPhrase("椅子");
		np.addPreModifier(adv1);
		np.addPreModifier(adv2);

		Assert.assertEquals("绿色 的 长 的 椅子", this.realiser
				.realise(np).getRealisation());

		adv1.setFeature(LexicalFeature.NO_DE, true);
        Assert.assertEquals("长 的 绿色 椅子", this.realiser
                .realise(np).getRealisation());
	}
