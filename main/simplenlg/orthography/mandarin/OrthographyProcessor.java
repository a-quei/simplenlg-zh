 ...
 */
package simplenlg.orthography.mandarin;
...
public class OrthographyProcessor extends NLGModule {
	...
	public NLGElement realise(NLGElement element) {
		...
		if(element != null) {
			...
			if(category instanceof DocumentCategory && element instanceof DocumentElement) {
				...
				if(DiscourseFunction.POST_MODIFIER.equals(function)) {// &&
					                                                         // appositive)
					                                                         // {
	 ...
	 * Adaptation for Chinese has been done, Changes:
		...
		for(int index = 0; index < length; index++ ) {
			...
			if(index < length - 2
			   && DiscourseFunction.CONJUNCTION.equals(realisedChild.getFeature(InternalFeature.DISCOURSE_FUNCTION))) {
                ...
                if (realisedChild.getFeatureAsBoolean(Feature.TOPIC)) {
