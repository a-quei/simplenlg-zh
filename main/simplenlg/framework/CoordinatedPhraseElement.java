...
public class CoordinatedPhraseElement extends NLGElement {
    ...
    public static boolean resetSpecifier = false;

	/** Coordinators which make the coordinate plural (eg, "and" but not "or")*/
	@SuppressWarnings("nls")
	private static final List<String> PLURAL_COORDINATORS = Arrays.asList("和");
	...
	public CoordinatedPhraseElement() {
		super();
		this.setFeature(Feature.CONJUNCTION, "和"); //$NON-NLS-1$
	}
	...
	public void addCoordinate(Object newCoordinate) {
		...
		if (coordinates == null) {
			...
			if (getFeatureAsBoolean(Feature.TOPIC)
                    && newCoordinate instanceof NLGElement) {
                ((NLGElement) newCoordinate).setFeature(Feature.TOPIC, true);
            }
	...
	public void topicalise() {
        List<NLGElement> coordinates = getFeatureAsElementList(InternalFeature.COORDINATES);
        if (coordinates != null) {
            coordinates.get(0).setFeature(Feature.TOPIC, true);
        } else {
            setFeature(Feature.TOPIC, true);
        }
    ...
    public void addSpecifier(Object specifier) {
        ...
        NLGElement firstChild = getChildren().get(0);
        NLGElement child = null;

        if (!resetSpecifier) {
            ((NPPhraseSpec) firstChild).clearSpecifiers();
        ...
        ((NPPhraseSpec) firstChild).addSpecifier(specifier);
    }
    ...
    public  void setSpecifier(Object specifier) {
        ...
        firstChild.setFeature(InternalFeature.SPECIFIERS, null);
        ((NPPhraseSpec) firstChild).addSpecifier(specifier);
    }
